...
def create_main_menu():
    ...
    buttons = ["🏠 Новый проект", "📚 Гайды", "📊 История", "⚙ Настройки"]
    markup.add(*buttons)
    return markup
...
def send_result_message(user_id, total, details):
    ...
    markup.row("📁 Сохранить проект", "🔙 Главное меню")
...
def confirm_clear_history(message):
    user_id = message.chat.id
    user = get_user_data(user_id)
...
@bot.message_handler(func=lambda m: m.text == "📁 Сохранить проект")
